...
counts <- read.csv(file="Data/Mcap_gene_count_matrix.csv", header=T, sep=",", row.names=1) #Load expression matrix from trinity
#rownames(counts) <-counts[,1]
#counts <- counts[,-1]
filt <- filterfun(pOverA(0.9,5)) #set filter values for PoverA, P percent of the samples have counts over A
...
annot <- read.csv("Data/200306_Mcap_annotations.csv", header=T, sep=",") #biological annotation information
map <- read.csv(file="Data/Mcap_stringtie_merged.gtf", header=FALSE, sep="\t", skip=2) #load sample info
map <- separate(map, V9, into = c("Gene.ID", "Transc.ID", "Genome.ID", "D", "E"), sep=";")
map <- subset(map, V3=="transcript")
map <- map[,c(1,4,5,9,11)]
map$Gene.ID <- gsub("gene_id ","",map$Gene.ID) #remove extra characters
map$Genome.ID <- gsub("ref_gene_id ","",map$Genome.ID) #remove extra characters
colnames(map) <- c("scaffold", "start", "stop", "Gene.ID", "Genome.ID")
map <- map %>% mutate_all(na_if," ")
uni.MSTRG <- map %>% count(Gene.ID) %>%
  left_join(map) %>% filter(n==1)

map.MSTRG <- map %>% count(Gene.ID) %>%
  left_join(map) %>% filter(n>1) %>% drop_na(Genome.ID) %>%
  bind_rows(uni.MSTRG)  

which(map.MSTRG$Gene.ID == "MSTRG.2800")
map.MSTRG[7133,]
which(annot$Gene.ID == "g34884")
annot[30306,]

#merge 5x file with annotations
expression <- as.data.frame(counts.5x)
expression$Gene.ID <- rownames(expression)
counts.5x.expression.annot <- expression %>% left_join(map.MSTRG)
...
DEG.int.res <- DEG.int.res[abs(DEG.int.res$log2FoldChange) >=1.5, ]
...
DEGs$Gene.ID <- rownames(DEGs)

#annot.DEGs <- DEGs %>% left_join(map.MSTRG)
#annot.DEGs <- annot.DEGs %>% left_join(annot)

write.csv(counts(sig.list), file="Output/DEG_5FDR.all.csv")

##### View Gene Expression Heatmap and PCA #####
...
plot(PC.info$PC1, PC.info$PC2, xlim=c(-12,12), ylim=c(-9, 9), xlab="PC1 55%", ylab="PC2 15%", col = c("lightpink2", "steelblue1","yellow3")[as.numeric(PC.info$Time)], pch=c(16, 17)[as.numeric(PC.info$Spawn)], cex=1.3)
legend(x="top", 
       bty="n",
...
DEG.Heat.res <- pheatmap(mat, color = colorRampPalette(c("darkblue", "cyan", "white", "yellow", "red"))(100), annotation_col=df, clustering_method = "average", scale="none",
         clustering_distance_rows="euclidean", show_rownames =FALSE, cluster_cols=TRUE, cluster_rows=TRUE,
...
set.seed(123) 
k.max <- 30 # Compute and plot wss for k = 2 to k = 20
